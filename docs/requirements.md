...
| FR-1.5 | `firewall-refresh` | 稼働中コンテナ内で `init-firewall.sh` を再実行（DNS 再解決）。`compose ps -q claude` の戻り値は**行ごとに配列へ取り込む**: 0 件なら `no running claude container` で exit 1、1 件ならそのコンテナで `init-firewall.sh` を実行、**2 件以上なら対象を推測せず exit 1**（複数 claude コンテナ同時起動時に複数 CID を単一引数として `docker exec` へ渡し引数展開が壊れるのを防ぐ。エラーメッセージで個別 `docker exec` による明示指定を案内する）。 |
...
| 日付 | 改訂内容 | 担当 |
| --- | --- | --- |
| 2026-06-01 | issue #10（P2）対応: `bin/aidock` の `cmd_firewall_refresh()` を複数コンテナ耐性化。`compose ps -q claude` の戻り値を `mapfile` で配列へ取り込み、0 件→exit 1 / 1 件→`init-firewall.sh` 実行 / 2 件以上→対象を推測せず exit 1（個別 `docker exec` を案内）に分岐。従来は複数 CID を単一スカラに格納し `docker exec "$cid"` で引数展開が壊れていた。FR-1.5 に複数コンテナ時の挙動を明記。 | Claude Code |
