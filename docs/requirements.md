...
- FR-8.1: **type-check ジョブ**は次の静的解析を実行し、いずれか失敗で CI を不合格とする。
  - `shellcheck`（v0.11.0、GitHub Releases から取得した固定版）を全シェルスクリプト（`bin/aidock`・`docker/init-firewall.sh`・`docker/entrypoint.sh`・`test/guard_test.sh`・`test/entrypoint_test.sh`）に適用。
  - `bash -n` による構文チェック（同じ 5 ファイルすべてに適用）。
...
| SEC-12.1 | CIDR を ipset に追加する前に正規表現 `^([0-9]{1,3}\.){3}[0-9]{1,3}/[0-9]{1,2}$` の一致を検証する。**実装済み**（`init-firewall.sh:461` の `CIDR_RE`）。 | `init-firewall.sh` |
