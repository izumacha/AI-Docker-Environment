...
- FR-4.5: GitHub `https://api.github.com/meta` から CIDR を取得し ipset へ追加。取得した CIDR は SEC-12.1（正規表現）/ SEC-12.2（octet 0-255・prefix 0-32 の範囲）の検証を **両方** 通過した場合にのみ追加する（いずれも実装済み）。範囲外の CIDR は warn ログを残してスキップする（FR-4.7 best-effort）。meta 取得の curl は **一過性の失敗に対し有界リトライ**（`--retry 3 --retry-delay 2 --retry-max-time 20 --retry-all-errors`）を行う（issue #13）。これは初回 DNS 解決失敗・瞬断・レート制限 429 等の transient error を再試行で吸収し、CIDR フォールバック欠落による（GitHub IP ローテーション時の）接続失敗余地を縮小する目的。`--retry-all-errors` を用いるのは、素の `--retry` が timeout / HTTP 408・429・5xx のみ、`--retry-connrefused` が ECONNREFUSED のみを対象とし、**本変更が明示的に狙う「初回 DNS 解決失敗」（curl exit 6）を再試行しない**ため（全エラーを再試行対象に含める必要がある）。`--retry-max-time 20` は**リトライの累積実時間を上限化**する（`--max-time` は各転送のみを制限しリトライ毎にリセットされる）。curl は 403/429 の `Retry-After` ヘッダ（GitHub secondary rate limit）に従って待機するため、上限が無いと server 指定の待機がコンテナ起動を引き延ばしうる。本キャップで起動時間を予測可能な範囲に収める。全試行を通じて解消しない失敗（403 等を含む）は従来どおり warn ログで継続しホスト名解決 IP に縮退する。レスポンス本文はコマンド置換ではなく **`-o <tmpfile>`（`/tmp` tmpfs）で受け、curl 成功（exit 0）後にのみ読み出す**: `--retry` は失敗試行の部分ボディを stdout に出力しうるため、コマンド置換だと部分バイトが連結し JSON が壊れて jq が CIDR を 0 件にする恐れがある（curl manual も redirected stdout からの retry 出力解析を非推奨と明記）。`-o` は試行毎にファイルを truncate するため成功時は最終応答のみが残る（codex P2 レビュー ×3 反映）。リトライを尽くしてもなお meta 取得自体が失敗した場合は **warn ログのみで継続**し、ホスト名解決で得た IP の範囲に縮退する（403 等のハードエラーは再試行対象外で従来どおりこの縮退パスに落ちる）。**実装済み**。
