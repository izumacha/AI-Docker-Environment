...
- `agent` から `127.0.0.1:53`（プロキシ）以外への直接 53 番送信は DROP され、プロキシ迂回での解決ができないこと（FR-11.2）。これには Docker 組込 DNS（`127.0.0.11:53`）への `agent` からの直接送信も含む（loopback 包括 ACCEPT で迂回できないこと。FR-11.2 / FR-4.2）。
