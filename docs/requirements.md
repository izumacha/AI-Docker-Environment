...
- FR-4.5: GitHub `https://api.github.com/meta` から CIDR を取得し ipset へ追加。取得した CIDR は SEC-12.1（正規表現）/ SEC-12.2（octet 0-255・prefix 0-32 の範囲）の検証を **両方** 通過した場合にのみ追加する（いずれも実装済み）。範囲外の CIDR は warn ログを残してスキップする（FR-4.7 best-effort）。meta 取得の curl は **一過性の失敗に対し有界リトライ**（`--retry 3 --retry-delay 2 --retry-max-time 20 --retry-connrefused`）を行う（issue #13）。これは初回 DNS 解決失敗・瞬断・レート制限 429 等の transient error を再試行で吸収し、CIDR フォールバック欠落による（GitHub IP ローテーション時の）接続失敗余地を縮小する目的。`--retry-max-time 20` は**リトライの累積実時間を上限化**する（`--max-time` は各転送のみを制限しリトライ毎にリセットされる）。curl は 403/429 の `Retry-After` ヘッダ（GitHub secondary rate limit）に従って待機するため、上限が無いと server 指定の待機がコンテナ起動を引き延ばしうる。本キャップで起動時間を予測可能な範囲に収める（codex P2 レビュー反映）。リトライを尽くしてもなお meta 取得自体が失敗した場合は **warn ログのみで継続**し、ホスト名解決で得た IP の範囲に縮退する（403 等のハードエラーは再試行対象外で従来どおりこの縮退パスに落ちる）。**実装済み**。
