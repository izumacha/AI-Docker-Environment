...
# **先にファイルが読めることを要求する**のが要点: grep はファイルが無いときも
# 「見つからなかった」と同じ非ゼロを返すため、素直に `if ! grep …` と書くと
# パスの打ち間違いや捕捉ファイルの作り忘れがそのまま PASS になる
# （このリポジトリが繰り返し踏んだ「不在＝合格」）。
assert_file_not_contains() {
    # 読めないファイルは「検査できなかった」ので失敗側へ倒す
    if [[ ! -r "$1" ]]; then
        report 1 "$3"
        return
    fi
    # 固定文字列として探し、見つからなければ成功
    if grep -qF -- "$2" "$1"; then
        report 1 "$3"
    else
        report 0 "$3"
