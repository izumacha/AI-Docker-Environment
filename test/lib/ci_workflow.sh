...
#
# **`strip_comment()` はこの定数を使わない**（意図的な例外）。あちらが見ているのは YAML ではなく
# `run:` の中の**シェル**で、コメントの規則もシェルのもの（引用の中の `#` はその場で
# `mask_quoted` を使って判別する）。同じ綴りに揃えると、YAML 側の都合でこの断片を変えたときに
# シェルの解釈まで巻き添えで動く。言語が違うものを 1 つにまとめない、という線引き
...
emit_structural_lines() {
    ...
    awk -v dq='"' -v sq="'" -v flow_re='[][{},#]' -v flow_mask='~' \
            ...
            for (i = 1; i <= len; i++) {
                ...
                if (quote == "") {
                    ...
                    if (ch == dq || ch == sq) {
                        ...
                        # 落とした場合も行頭の印の並びは終わっている（上の閉じ側と同じ理由）
                        seqzone = 0
                        continue
                    }
        ...
        BEGIN { in_scalar = 0; scalar_indent = 0 }
        {
            ...
            # `#` の直前に空白か行頭を要求するのは他の 2 か所と同じ規則（YAML でコメントが
