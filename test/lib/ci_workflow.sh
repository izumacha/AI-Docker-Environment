...
emit_structural_lines() {
    ...
    awk -v dq='"' -v sq="'" -v flow_punct='[]{},' -v flow_mask='~' -v quote_openers=':,{[-' '
        ...
        #
        # なぜ 1 文字を 1 文字へ置き換えるのか: 消費側は版注記を探すために「値が行の何文字目で
        # 終わるか」を数えている。長さが変わる置き換えをすると、その桁がずれて注記を取り違える。
        #
        # **引用符なら何でもスカラーの開始と見なしてはならない（fail-open になる）。**
        # 素のスカラーの途中に現れるアポストロフィ（`don'"'"'t` など）は YAML では**ただの文字**で、
        # これを開始と読むと、次のアポストロフィ（`it'"'"'s` など）までが「引用の中」になる。
        # その範囲に**本物の**区切りが挟まっていると、それごと伏せて `uses:` を見逃す
        # （実測: `- {name: don'"'"'t, uses: actions/evil@v1, desc: it'"'"'s fine}` で
        # `actions/evil@v1` の可変タグが検査から丸ごと消え、main は検出していた）。
        # そこで**値が始まりうる位置に置かれた引用符だけ**を開始と認める（YAML でも、引用された
        # スカラーは鍵の後・フローの区切りの後・並びの `- ` の後にしか始まらない）。
        # 位置が合わない引用符は、従来どおりただ落とすだけにする＝今日と同じ結果に倒れる。
        #
        # 引用符そのものは今までどおり落とす（`"permissions":` と `permissions:` を同じ形に
        # 揃える FR-8.1 の前提。引用の中に入れ子で現れた別種の引用符も同じく落とす）。
        function mask_quoted_flow_punctuation(src,   out, held, raw, quote, prev, i, ch, len) {
            # 確定した出力・引用の中で伏せた写し・伏せる前の写し・開いている引用符の種類を初期化する
            out = ""; held = ""; raw = ""; quote = ""
            # 引用の外で最後に見た空白以外の文字（開始位置の判定に使う。行頭なら空のまま）
            prev = ""
            ...
            for (i = 1; i <= len; i++) {
                ...
                if (quote == "") {
                    # 引用符かどうかを先に見る
                    if (ch == dq || ch == sq) {
                        # 値が始まりうる位置（行頭 / 鍵の `:` の後 / フローの区切りの後 / `- ` の後）
                        # に置かれているときだけ、スカラーの開始として扱う
                        if (prev == "" || index(quote_openers, prev) > 0) {
                    ...
                    out = out ch
                    # 空白以外なら「直前の文字」として覚える（空白は位置判定に影響させない）
                    if (ch !~ /[[:space:]]/) prev = ch
                    continue
                }
                # ここからは引用の中。同じ種類の引用符が来たらスカラーの終わり
                if (ch == quote) {
                    # 伏せた写しを出力へ確定させ、引用の外へ戻る
                    out = out held
                    # 閉じた直後に続く引用符を「開始」と誤読しないよう、開始位置になりえない文字を
                    # 直前の文字として置く（中身があればその末尾、空なら `"` を代用する）
                    prev = (length(held) > 0) ? substr(held, length(held), 1) : dq
                    # 引用の状態を片付けて次の文字へ進む
                    quote = ""; held = ""; raw = ""
                    continue
                }
            ...
            # 引用の対応が取れない行（散文中のアポストロフィなど）で伏せてしまうと、
            # **本物の区切りを隠して `uses:` を見逃す**恐れがあるため、従来の挙動へ倒す（fail-closed）
