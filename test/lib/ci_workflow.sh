            ...
            if (!step_disabled) {
                ...
                for (i = 1; i <= njoined; i++) {
                    ...
                    for (j = 1; j <= nseg; j++) {
                        ...
                        # （fail-open。レビューで実測）
                        arm_expected = (opens_case && !set_arm_here)
