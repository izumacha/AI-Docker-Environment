...
ホストの `~/.ssh` や `~/.aws` 等を **追加 bind mount しません**。ただし `$PWD` は `/workspace:rw` として渡されるため、機密ディレクトリ配下では `aidock` は起動を機械的に拒否する（詳細は `docs/requirements.md` SEC-8）。Linux 専用（macOS/Windows 非対応）。
