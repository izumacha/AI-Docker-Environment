...
ホストの `~/.ssh` や `~/.aws` 等は一切マウントしません。Linux 専用（macOS/Windows 非対応）。
...
- CI/CD は **codex 自動レビュー用の `.github/workflows/codex-review.yml`** のみ。その他のジョブ（lint/test）は未設定。
- pre-commit フックは未設定
- コミットメッセージは英語・命令形・1 行要約（既存履歴に倣う）
- 開発は機能ブランチで行い、`main` への直 push は避ける
- **PR を作成・更新したら必ず `mcp__github__add_issue_comment` で `@codex review` を投稿する**。`.github/workflows/codex-review.yml` も自動投稿するため二重起動になるが、codex 側の dedup を期待する。draft PR でも明示コメントがあれば codex がレビューを開始する。
