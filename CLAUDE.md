...
- CI は GitHub Actions（`.github/workflows/ci.yml`）で **型チェック**（shellcheck / hadolint / `docker compose config`）と **e2e**（受け入れ基準 AC-1〜AC-4 / AC-7 の実機検証）を実行する。詳細は `docs/requirements.md` の FR-8 / AC-8。CI からは codex へコメント投稿しない（FR-7）
- pre-commit フックは未設定
